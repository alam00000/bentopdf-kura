...
QPDFObjectHandle fontFileStream(QPDFObjectHandle fd) {
...
bool loadFace(FtLib& lib, QPDFObjectHandle program, FtFace& out) {
  try {
...
bool parseUniName(const std::string& name, uint32_t& out) {
...
SimpleEncoding readEncoding(QPDFObjectHandle font, bool symbolic) {
  SimpleEncoding enc;
...
FT_UInt glyphForCode(FT_Face face, int code, const SimpleEncoding& enc, bool symbolic) {
...
class CffWidths {
 public:
  explicit CffWidths(const std::string& file) {
    ...
    auto getOff = [&](int op) -> size_t {
      ...
      return it != topDict.end() && !it->second.empty()
                 ? base + static_cast<size_t>(it->second.back())
                 : 0;
    };
 ...
 private:
  ...
  std::pair<double, double> readPrivate(std::map<int, std::vector<double>>& dict, size_t base) {
    ...
    size_t psz = static_cast<size_t>(it->second[0]);
    size_t poff = base + static_cast<size_t>(it->second[1]);
    if (data + poff + psz > end) return {0.0, 0.0};
